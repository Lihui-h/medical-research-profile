# src/crawlers/social_crawlers/slgjyxy/baidu_search_crawler.py
import os
import sys
import random
import time
...
from src.crawlers.social_crawlers.slgjyxy.keyword_generator import KeywordGenerator  # 新增导入
...
class TiebaSpider:
    """百度贴吧爬虫（增强代理稳定性版）"""
    #初始化方法
    def __init__(self, kw='树兰国际医学院', max_page=2, delay=10):
        # 初始化参数
        ...
        for post in post_list:
            ...
            try:
                ...
                item.update({
                    #固定字段
                    'source': 'baidu_tieba',
                    'institution_name': '树兰国际医学院',
                    'org_code': 'slgjyxy',
                    'user_id': os.getenv("SUPABASE_USER_UUID_SLGJYXY"),
                })
                ...
                # ==== 有效性校验和广告过滤 ====
                def is_advertisement(title, content):
        
                    ad_keywords = [
                        "代考", "代办", "代理", "招生", "兼职", "公会提成", "奢侈大牌"
                        "三角洲", "出租", "流量", "跑单", "手机维修", "暖宝宝贴厂家",
                        "低价", "优惠", "促销", "转让", "出售", "求购", "活动赞助",
                    ]

                    if any(kw in title or kw in content for kw in ad_keywords):
                        return True
                ...
                valid_check = [
                    ...
                    (len(item['title'])>5 or len(raw_content)>7),  # 标题和内容长度检查
                    not is_advertisement(item['title'], raw_content),  # 广告过滤
                    any(kw in raw_content for kw in self.keyword_tool.base_keywords) or 
                    any(kw in item['title'] for kw in self.keyword_tool.base_keywords) or
                    any(kw in item['forum'] for kw in self.keyword_tool.base_keywords), # 内容、标题、贴吧名称有一个包含关键词即可
                ]
                if not all(valid_check):
                    ...
                    if not valid_check[1]:
                        reasons.append(f"内容或标题过短") 
                    if not valid_check[2]:
                        reasons.append(f"疑似广告")
                    if not valid_check[3]:
                        reasons.append(f"无关键词匹配")
    ...
    def generate_search_urls(self):
        """生成带分页的搜索URL"""
        urls = []
        base_search_url = "https://tieba.baidu.com/f/search/res"
        for keyword in self.keyword_tool.generate():  # 获取原始关键词
            ...
            for page in range(0, 10):
                ...
                urls.append(f"{base_search_url}?{query}")
